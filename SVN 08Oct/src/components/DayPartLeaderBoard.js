...
import {TouchableOpacity, Platform, TouchableHighlight,RefreshControl, StatusBar, AsyncStorage, Dimensions,Image,FlatList, Keyboard, Text, View, Alert, TextInput} from 'react-native';
...
import {storeVasu} from "./../utilities/GenericMessages"
...
import { getDayPartStores } from "./../services/webServices"
...
let isCheckedData = []
export default class DayPartLeaderBoard extends React.Component {
  constructor(props) {
    super(props)
    this.state ={
      stores :[],
  ...
  /*WebService to get DayPart store data*/
  getDayPartStoreData = ()=> {
    this.setState({isAnimating:true})
    this.setState({refreshing: true},()=>{this.props.setShowSearchState()})
    getDayPartStores(this.state.uName, this.state.JWT)
    .then((response) => {
      if((response.status==CODE_200)  && response.body.length!==0) {
        ...
        if(this.state.column2Select =="Total") {
          intialStores.sort( function ( a, b ) { return b.Daypart.Total - a.Daypart.Total } )
          responseData.sort( function ( a, b ) { return b.Daypart.Total - a.Daypart.Total } )
        }
        else {
          intialStores.sort( function ( a, b ) { return b.Daypart.Car - a.Daypart.Car } )
          responseData.sort( function ( a, b ) { return b.Daypart.Car - a.Daypart.Car } )
        }
...
/************************************************************************************/
  /*Get list of rows of daypart Data*/
  renderRow(rowData, rowID) {
    let minutes = Math.floor((rowData.Daypart.Total) / 60)
    let seconds = ((rowData.Daypart.Total)%60) >= 10 ? ((rowData.Daypart.Total)%60) : "0"+((rowData.Daypart.Total)%60)
    ...
    return (
        ...
        <View style={[CommonStyles.flatListRendeRow,CommonStyles.borderBottom1,
          ...
          <View style={[CommonStyles.flexOne]}>
            {/*Managing list data according to column name*/
              ...
              ?
                <Text allowFontScaling ={false} ellipsizeMode={'tail'}
                  numberOfLines={1}
                  style={[CommonStyles.flatListRenderRowTextColor,
                          CommonStyles.textLeft,
                          CommonStyles.font20,
            ...
            ?
              <Text allowFontScaling ={false}
                style={[CommonStyles.textRight,
                        ...
                        CommonStyles.font20,
                        CommonStyles.mR10]}>
                {this.state.timeFormat == "seconds" ? rowData.Daypart.Total : timeInMinutes }
              </Text>
            :
  ...
  setAscendingFlag2 = (columnName) => {
    const {stores, column2Select} = this.state
      ...
      if(this.state.isAscendingCol2===0){
        ...
        this.setState({stores:stores.sort( function ( a, b ) { return b.Daypart[columnName] - a.Daypart[columnName] })})
      }
          ...
          }
          {
            ...
            :
              <View style={[CommonStyles.noDataTextStyle]}>
                <Text allowFontScaling ={false}
                  style={[CommonStyles.font16,CommonStyles.fontBold]}>
                  {NO_DATA()}
                </Text>
              </View>
          }
