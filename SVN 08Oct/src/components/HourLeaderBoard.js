...
import {TouchableOpacity,Platform, StatusBar,TouchableHighlight, RefreshControl, AsyncStorage, Dimensions,Image,FlatList, Keyboard, Text, View, Alert, TextInput} from 'react-native';
...
import {stores, getHourStores} from "./../services/webServices"
...
export default class HourLeaderBoard extends React.Component {
  constructor(props) {
    super(props)
    this.state ={
      stores :[],
  ...
  /*WebService to get Hour store data*/

  getHourStoreData = ()=> {
    this.setState({isAnimating:true})
    this.setState({refreshing: true},()=>{this.props.setShowSearchState()})
    getHourStores(this.state.uName, this.state.JWT)
    .then((response) => {
      if((response.status==CODE_200)  && response.body.length!==0) {
        ...
        if(this.state.column2Select =="Total") {
          intialStores.sort( function ( a, b ) { return b.Hour.Total - a.Hour.Total })
          responseData.sort( function ( a, b ) { return b.Hour.Total - a.Hour.Total })
        }
        else {
          intialStores.sort( function ( a, b ) { return b.Hour.Car - a.Hour.Car })
          responseData.sort( function ( a, b ) { return b.Hour.Car - a.Hour.Car })
        }
...
/************************************************************************************/
  /*Get list of rows of Hour Data*/
  renderRow(rowData, rowID) {
      let minutes = Math.floor((rowData.Hour.Total) / 60)
      let seconds = ((rowData.Hour.Total)%60) >= 10 ? ((rowData.Hour.Total)%60) : "0"+((rowData.Hour.Total)%60)
          ...
          {/*Managing list data according to column name*/
            ...
            ?
              <Text allowFontScaling ={false} style={[CommonStyles.textRight,
                        ...
                        CommonStyles.font20,
                        CommonStyles.mR10]}>
                {this.state.timeFormat == "seconds" ? rowData.Hour.Total : timeInMinutes }
              </Text>
            :
...
/************************************************************************************/
  ...
  setAscendingFlag2 = (columnName) => {
    ...
    if(this.state.isAscendingCol2===0){
      ...
      this.setState({stores:stores.sort( function ( a, b ) { return b.Hour[columnName] - a.Hour[columnName] })})
    }
